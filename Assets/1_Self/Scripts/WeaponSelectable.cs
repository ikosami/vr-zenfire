...
public class WeaponSelectable : MonoBehaviour
{
    ...
    void Update()
    {
        if(_interactable.isHovered) {
            if(_inputActionAssetLeft.action.ReadValue<float>() > 0.5f) {
                VibrationController.instance.Play("click", VibrationController.ControllerSide.Left);
                SelectWeapon();
            }
            if(_inputActionAssetRight.action.ReadValue<float>() > 0.5f) {
                VibrationController.instance.Play("click", VibrationController.ControllerSide.Right);
                SelectWeapon();
            }
        }
    }
    ...
    public void SelectWeapon() {
        WeaponSelector.Instance.SelectWeapon(_index);
    }
}
