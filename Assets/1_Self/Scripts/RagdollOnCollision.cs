        ...
        if (_collisionLayer == (_collisionLayer | (1 << collision.gameObject.layer)))
        {
            ...
            if (hit)
            {
                if(CanRagdoll == RagdollType.Combo) {
                    ...
                    prefab.Play();
                    VibrationController.instance.Play("click", VibrationController.ControllerSide.Both);
                ...
                else if (collision.relativeVelocity.magnitude >= _hitMagnitude)
                {
                    ...
                    VibrationController.instance.Play("hit_human", VibrationController.ControllerSide.Both);
                    foreach (var trail in _trailRenderer)
                    {
                        trail.emitting = true;
                    }
