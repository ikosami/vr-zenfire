        ...
        if (_collisionLayer == (_collisionLayer | (1 << collision.gameObject.layer)))
        {
            ...
            if (hit)
            {
                ...
                else if (collision.relativeVelocity.magnitude >= _hitMagnitude)
                {
                    ...
                    _rigidbody.linearVelocity = new Vector3(0,2.8f, VelocityRate * 7f + Mathf.Clamp(collision.impulse.z, 0, 10) * 10f * PlayerData.Instance.GetLv(ParamType.Power).Value * ParameterController.Instance.PunchFactor);
