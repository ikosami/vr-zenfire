...
namespace RayFire
{
    ...
    public class RFHexagon
    {
        ...
        public static List<Vector3> GetHexPointCLoudV1 (RFHexagon hexagon, Transform tm, Vector3 centerPos, Quaternion centerDirection, int seed, Bounds bound)
        {
        ...
        // Get final point cloud for hexgrid fragmentation
        public static List<Vector3> GetHexPointCLoudV2 (RFHexagon hexagon,Vector3 centerPos, Quaternion centerDirection, Bounds bound)
        {
            // Generate hex grid point cloud
            SetHexOutputCloud (hexagon);
            
            // Matrix multiply
            Quaternion quat = centerDirection;
            if (hexagon.plane == PlaneType.XY)
                quat = quat * Quaternion.Euler(90, 0, 0);
            if (hexagon.plane == PlaneType.YZ)
                quat = quat * Quaternion.Euler(0, 0, 90);
        ...
        {
            // Expand bound to cover points slightly outside
            Bounds bnd = new Bounds(bound.center, bound.size);
            bnd.Expand (bound.size.magnitude * 0.1f);
            ...
            // Filter points by bound
            for (int i = hexagon.pc.Count - 1; i >= 0; i--)
                if (bnd.Contains(hexagon.pc[i]) == false)
