...
It will probably work well enough for you to slap RASCAL on your mesh without any updating and be fine.
...
If you're not interested in being able to update the colliders at runtime, you can use the "Generate To Files" in the component's context menu, which will generate the meshes to files so they can be used in prefabs and such.
Though this comes at the cost of not being able to update colliders at runtime.
...
If you are working in a situation where your colliders need to be on a separate skeleton, such as in Puppet Master, you can use the included example script "RetargetSkeleton".
This component should be placed on the same object with the RASCAL component.
...
This can be used for testing to see how the colliders will be generated in the editor.
Or you could just generate colliders once in the editor and they will probably be saved to the scene itself, but not prefabs, which may be good enough for some applications.
These wouldn't be updatable or usable with any other of the RASCAL features, however.

There is also the "Generate To Files" in the component's context menu, which will generate the meshes to files so they can be used in prefabs and such.
This comes at the cost of not being able to update colliders at runtime.
