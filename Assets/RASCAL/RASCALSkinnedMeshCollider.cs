...
public class RASCALSkinnedMeshCollider : MonoBehaviour {
    ...
    static bool saveColMeshToFile;
    static string colMeshSaveLoc;
...
#if UNITY_EDITOR
    [ContextMenu("Generate To Files")]
    void MenuGenerateSave() {
        saveColMeshToFile = true;
        colMeshSaveLoc = UnityEditor.EditorUtility.SaveFilePanelInProject("Choose Save Location", "ColMesh", "asset", "RASCAL Collider Meshes");

        print(colMeshSaveLoc);
        if (string.IsNullOrWhiteSpace(colMeshSaveLoc)) {
            return;
        }
    ...
    public class BoneMeshColl {
        ...
        [NonSerialized] internal bool isPastThreshold = true;
        [NonSerialized] internal int collMeshInstID = -1;

        [NonSerialized] internal bool convex;
        ...
        void HandlePhysMatInheritence() {
            ...
            if(parentBone.materialProperties) {
                if(parentBone.materialProperties.overrideOtherMats) {
                    meshCol.sharedMaterial = parentBone.materialProperties.physicsMaterial;
                }
        ...
        public void Update(List<Vector3> bakedVerts, bool force = false) {
            ...
            if(thresh) {
                ...
                if (saveColMeshToFile) {
                    string fileName = Path.GetFileNameWithoutExtension(colMeshSaveLoc) + "-" + collMesh.name + ".asset";
                    string dir = Directory.GetParent(colMeshSaveLoc).FullName;
                    dir = "Assets" + dir.Substring(Application.dataPath.Length);
                    dir += "\\" + fileName;
                    savedColMeshName = dir;
#if UNITY_EDITOR
                    UnityEditor.AssetDatabase.CreateAsset(collMesh, dir);
#endif
        ...
        public void LoadFromDisk() {            
            var loadMesh = UnityEditor.AssetDatabase.LoadAssetAtPath<Mesh>(savedColMeshName);
            meshCol.sharedMesh = loadMesh;
            UnityEditor.EditorUtility.SetDirty(meshCol);
        }
#endif
        ...
        internal async void BakeMeshThreaded() {
            collMesh.vertices = transformedVerts;
