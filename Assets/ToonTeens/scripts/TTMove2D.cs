...
public class TTMove2D : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        //LOOK & MOVE
        if (turn) Chturn();
        if (grounded) rigid.linearVelocity = (new Vector3(anim.GetFloat("speed")*right, 0f, 0f));
        ...
        if (!grounded) rigid.linearVelocity += new Vector3(Input.GetAxisRaw("Horizontal")*0.025f, 0f, 0f);
    }
