...
public class HandPoseManager : MonoBehaviour
{
    ...
    private Coroutine[] _fingerAnimateCoroutines;
    ...
    void Start()
    {
        _fingerAnimateCoroutines = new Coroutine[fingerRigs.Length];
    ...
    public void ResetPose(HandPartType handPartType)
    {
        SelectPose(ResetPoseName, handPartType, duration: AnimateDuration);
    }
    ...
    void OnHandPoseChange(IEvent ev)
    {
        if (ev is Ev.Events.HandPoseChange handPoseChange)
        {
            ...
            {
                NS.ED.Log("OnHandPoseChange: " + handPoseChange.PoseName + " ControllerSide:" + ControllerSide);
                SelectPose(handPoseChange.PoseName, handPoseChange.HandPartType, duration: 0.2f);
            }
        }
    }
    ...
    {
        if (ev is Ev.Events.HandPoseReset handPoseReset)
        {
            if (handPoseReset.ControllerSide == ControllerSide)
            {
                ResetPose(handPoseReset.HandPartType);
            }
        }
    }
    ...
    public void SelectPose(string poseName, HandPartType handPartType, bool isImmediate = false, float duration = 0.5f)
    {
        ...
        if (_fingerAnimateCoroutines != null)
        {
            for (int i = 0; i < _fingerAnimateCoroutines.Length; i++)
            {
                if (_fingerAnimateCoroutines[i] != null && (handPartType & fingerRigs[i].handPartType) != 0)
                {
        ...
        else
        {
            ...
            for (int i = 0; i < fingerRigs.Length; i++)
            {
                ...
                // 指定されたHandPartTypeに含まれる場合のみアニメーションを開始
                if (fingerRig != null && fingerRig.root != null && (handPartType & fingerRig.handPartType) != 0)
                {
                    var fingerTransforms = GetFingerTransforms(fingerRig);
                    var fingerPoseData = GetFingerPoseData(CurrentPose, fingerRig.key);
    ...
    private Dictionary<string, Transform> GetChildTransforms(Transform transform)
    {
        ...
        foreach (Transform child in transform)
        {
            ...
            {
                if (transforms.ContainsKey(kvp.Key))
                {
                    continue;
                }
    ...
    private void ApplyPoseToTransforms(Dictionary<string, Transform> toTransforms, HandPoseScriptable fromPose)
    {
        ...
        {
            if (!toTransforms.ContainsKey(rig.ObjectName))
            {
                continue;
            }
    ...
    private List<HandRigData> GetFingerPoseData(HandPoseScriptable pose, string key)
    {
        return pose.Rigs.FindAll(rig => rig.ObjectName.StartsWith(key));
    }
