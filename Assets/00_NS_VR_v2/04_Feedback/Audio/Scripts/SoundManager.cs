...
/// <summary>
/// サウンド管理システム
/// BGMと効果音の再生、3D空間での音声再生、ピッチと音量の制御を行う
/// </summary>
public class SoundManager : NS.Util.SingletonMonoBehaviour<SoundManager>
{
    ...
    void Start()
    {
        ...
        var sound = SoundSets.Find(x => x.Name == _bgmName);
        if (sound != null)
        {
            _bgmAudio.SetAudioClip(sound.Sound);
            PlayBGM();
        }
        PlayBGM();
    ...
    [SerializeField] private AudioSourceCustom[] _audio3DPrefabs;

    /// <summary>
    /// 3D空間で音声を再生
    /// 指定した位置に音源を生成し、空間的な音声を提供
    /// </summary>
    /// <param name="name">再生するサウンド名</param>
    /// <param name="position">再生する位置</param>
    /// <param name="index">使用するプレハブのインデックス（デフォルト: 0）</param>
    /// <returns>生成されたAudioSourceCustomインスタンス</returns>
    public AudioSourceCustom Play3D(string name, Vector3 position, int index = 0, float time = 0f)
    {
        // 指定位置に3Dオーディオソースを生成
        var audioSourceCustom = Instantiate(_audio3DPrefabs[index], position, Quaternion.identity);
        audioSourceCustom.SetClip(name);
        audioSourceCustom.SetTime(time);
        audioSourceCustom.Play();
        return audioSourceCustom;
    }
}
