...
public class PhysicsHandMoveManager : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        {
            totalForce += _leftHandPhysics.CalculateCollisionResponseForce(_playerRigidbody.transform);
        }

        // 右手の衝突応答力を計算
        if (_isRightHandColliding && _rightHandPhysics != null)
        {
            totalForce += _rightHandPhysics.CalculateCollisionResponseForce(_playerRigidbody.transform);
        }
